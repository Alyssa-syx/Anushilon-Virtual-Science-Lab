...
const Statistics = () => {
  ...
  useEffect(() => {
    let unsubscribe1;
    unsubscribe1 = db.collection('users').onSnapshot((snapshot) => {
  ...
  useEffect(() => {
    let unsubscribe2;
    unsubscribe2 = db.collection('classes').onSnapshot((snapshot) => {
      setLab(snapshot?.docs?.length);
    });
  ...
  useEffect(() => {
    let unsubscribe3;
    unsubscribe3 = db.collection('posts').onSnapshot((snapshot) => {
  ...
  useEffect(() => {
    let unsubscribe4;
    unsubscribe4 = db.collection('workshop').onSnapshot((snapshot) => {
      setWorkshop(snapshot?.docs?.length);
    });
  ...
  return (
    <>
      {user === 0 && post === 0 && lab === 0 && workshop === 0 ? (
        <div className="py-12">
          <div className="flex animate-pulse flex-row items-center h-full justify-center mb-6">
            <div className="w-128 bg-indigo-50 h-16 sm:h-10 rounded-md "></div>
          </div>
          <div className="container w-full grid grid-cols-1 gap-6 m-4 mx-auto md:m-0 md:grid-cols-2 xl:grid-cols-3 font-body">
            <div className="flex animate-pulse flex-row items-center h-full justify-center">
              <div className="w-full bg-indigo-50 h-12 sm:h-16 rounded-md mx-6"></div>
            </div>
            <div className="flex animate-pulse flex-row items-center h-full justify-center">
              <div className="w-full bg-indigo-50 h-12 sm:h-16 rounded-md mx-6"></div>
            </div>
        ...
        <section className="p-4 my-6 md:p-8 bg-indigo-50 text-gray-800">
          ...
          <div className="container grid grid-cols-1 gap-6 m-4 mx-auto md:m-0 md:grid-cols-2 xl:grid-cols-3 font-body">
            <div className="flex overflow-hidden rounded-lg bg-indigo-100 text-gray-800">
              <div className="flex items-center justify-center px-4 bg-violet-600 text-indigo-100">
                <svg
